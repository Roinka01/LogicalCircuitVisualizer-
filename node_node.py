...
class Node(Serializable):
    ...
    def __init__(self, scene: 'Scene',gate:Gate=None ,
        ...
        InAndOutList=" In: "+gate.getListEntries()+"\n Out: "+gate.getOutput()
    ...
    def initSockets(self, inputs: list, outputs: list, reset: bool=True):
        ...
        inp=[]
        if (self.gate.getGateType().lower()=='not'):
            inp.append(inputs[0])
        elif (self.gate.getGateType().lower()!='input'):
            inp=inputs
        no_of_sockets_in_gate=len(inp)
        for item in inp:
            socket = self.__class__.Socket_class(
                ...
                count_on_this_node_side=no_of_sockets_in_gate, is_input=True
            )
            counter += 1
